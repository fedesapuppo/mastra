...
export type HarnessCompatMode = HarnessMode & { agentId?: string };
...
export class HarnessCompat<TState = {}> {
  ...
  #agentForMode(mode: HarnessMode): Agent {
    const agentId = (mode as HarnessCompatMode).agentId;
    if (agentId) return this.#config.mastra.getAgentById(agentId) as Agent;
    if (this.#config.defaultAgent) return this.#config.defaultAgent;
    throw new Error(`Harness V1 mode "${mode.id}" does not declare an agentId`);
  }
